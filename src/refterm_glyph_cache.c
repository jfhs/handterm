...
gpu_glyph_index PackGlyphCachePoint(uint32_t X, uint32_t Y)
{
...
int GlyphHashesAreEqual(glyph_hash A, glyph_hash B)
{
...
uint32_t *GetSlotPointer(glyph_table *Table, glyph_hash RunHash)
{
...
glyph_entry *GetEntry(glyph_table *Table, uint32_t Index)
{
...
glyph_entry *GetSentinel(glyph_table *Table)
{
...
#if DEBUG_VALIDATE_LRU
void ValidateLRU(glyph_table *Table, int ExpectedCountChange)
{
...
void RecycleLRU(glyph_table *Table)
{
    glyph_entry *Sentinel = GetSentinel(Table);
...
uint32_t PopFreeEntry(glyph_table *Table)
{
    glyph_entry *Sentinel = GetSentinel(Table);
